...
import { Headphones, Lock, MapPin } from "lucide-react";
...
export function MusicLibrary({ items }: { items: SetItem[] }) {
        ...
        <article className="set-card" key={item.id}>
          ...
          <div className="set-copy">
            ...
            {item.tracklist?.length ? (
              <details>
                <summary>Ver tracklist</summary>
                <ol className="tracklist">
                  {item.tracklist.map((track, index) => (
                    <li key={`${track.time}-${index}`}>
            ...
            <div className="inline-actions">
              <Link className="button secondary" href={`/musica/${item.slug}`}>
                Ver página
              </Link>
              {item.demo ? (
              ...
              {item.externalUrl && !item.demo ? (
                <a className="button secondary" href={item.externalUrl} target="_blank" rel="noreferrer">
                  Plataforma oficial
                </a>
              ) : null}
            </div>

...
export async function signInWithPassword(formData: FormData) {
  ...
  const {data:setting}=await supabase.from("site_settings").select("value").eq("key","disable_login").eq("is_public",true).maybeSingle();
  if(systemEnabled({disable_login:setting?.value},"disable_login"))redirect(`/acceso?error=${encodeURIComponent(systemMessage({disable_login:setting?.value},"disable_login","Login is temporarily unavailable."))}`);
...
export async function signUpWithPassword(formData: FormData) {
  const {email,password,next}=getCredentials(formData); const supabase=await createClient();
  if(!supabase) redirect("/acceso?error=config");
  const {data:setting}=await supabase.from("site_settings").select("value").eq("key","disable_pass_registration").eq("is_public",true).maybeSingle();
  if(systemEnabled({disable_pass_registration:setting?.value},"disable_pass_registration"))redirect(`/acceso?error=${encodeURIComponent(systemMessage({disable_pass_registration:setting?.value},"disable_pass_registration","Pass registration is temporarily closed."))}`);

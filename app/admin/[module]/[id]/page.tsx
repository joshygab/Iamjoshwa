...
import { PublicLinkActions } from "@/components/admin/public-link-actions";
...
export default async function EditContent({params}:{params:Promise<{module:string;id:string}>}){
  ...
  const[{data},{data:assetRows}]=await Promise.all([supabase.from(table).select("*").eq("id",id).maybeSingle(),supabase.from("media_assets").select("id,display_name,mime_type,byte_size,bucket,storage_path").eq("bucket","public-media").is("archived_at",null).order("display_name")]);
  if(!data)notFound();
  ...
  const assets=(assetRows||[]).map(asset=>({id:asset.id,display_name:asset.display_name,mime_type:asset.mime_type,byte_size:asset.byte_size,public_url:supabase.storage.from("public-media").getPublicUrl(asset.storage_path).data.publicUrl}));
  const publicHref=getPublicHref(module,String(data.slug||""));
  const isPublished=String(data.publication_status||data.status)==="published";
  return <><header className="admin-header"><div><span className="section-kicker">EDITAR</span><h1>{data.name||data.title||data.section_key}</h1><p>{isPublished?"Página pública activa.":"Todavía no aparece públicamente porque no está publicado."}</p></div>{publicHref?<div className="admin-public-header-actions"><span className={isPublished?"visible-dot":"hidden-dot"}>{isPublished?"Visible":"Oculto"}</span><PublicLinkActions href={publicHref}/></div>:null}</header><ContentEditor module={module} initial={data} assets={assets}/>{versions?.length?<details className="settings-card version-list"><summary>Historial y restauración</summary>{versions.map(version=><form action={restoreVersion} key={version.id}><input type="hidden" name="versionId" value={version.id}/><p>Versión {version.version} · {formatMxDateTime(version.created_at)} MX</p><button className="button secondary">Restaurar</button></form>)}</details>:null}</>;
}
...
function getPublicHref(module:string,slug:string){if(!slug)return"";if(module==="eventos")return`/fechas/${slug}`;if(module==="lanzamientos")return`/lanzamientos/${slug}`;if(module==="sets")return`/musica/${slug}`;return""}

  ...
  return (
    <>
      ...
      <section className="admin-table audit-log-table">
        ...
        {(data || []).map((row) => <article key={row.id}><div><strong>{row.action} · {row.entity_type}</strong><small>{new Date(row.created_at).toLocaleString("es-MX")} · {row.entity_id}</small></div><details><summary>Ver valores</summary><pre>{JSON.stringify({ old: row.old_values, new: row.new_values }, null, 2)}</pre></details></article>)}
      </section>
    </>
  );
}

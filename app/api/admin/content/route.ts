...
async function authorize() {
  ...
  return { db, writeDb: createAdminClient(), user };
}
...
async function save(request: Request) {
  ...
  const query = body.id ? auth.writeDb.from(table).update({ ...payload, updated_at: new Date().toISOString() }).eq("id", body.id) : auth.writeDb.from(table).insert(insertPayload);
  const { data, error } = await query.select("id").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  if (releaseLinks) {
    const { error: removeError } = await auth.writeDb.from("release_links").delete().eq("release_id", data.id);
    if (removeError) return NextResponse.json({ error: removeError.message }, { status: 400 });
    if (releaseLinks.length) { const { error: linksError } = await auth.writeDb.from("release_links").insert(releaseLinks.map((link) => ({ ...link, release_id: data.id }))); if (linksError) return NextResponse.json({ error: linksError.message }, { status: 400 }); }
  }
  if (setTracks) {
    const { error: removeError } = await auth.writeDb.from("set_tracks").delete().eq("set_id", data.id);
    if (removeError) return NextResponse.json({ error: removeError.message }, { status: 400 });
    if (setTracks.length) { const { error: tracksError } = await auth.writeDb.from("set_tracks").insert(setTracks.map((track) => ({ ...track, set_id: data.id }))); if (tracksError) return NextResponse.json({ error: tracksError.message }, { status: 400 }); }
  }
  if (contentModule !== "campanas" && requestedStatus === "scheduled" && publishAt) await auth.writeDb.from("publication_schedule").insert({ entity_type: table, entity_id: data.id, action: "publish", execute_at: publishAt, created_by: auth.user.id });
  await auth.writeDb.from("audit_logs").insert({ actor_id: auth.user.id, action: body.id ? "update" : "create", entity_type: table, entity_id: data.id, old_values: old, new_values: payload });
...
export async function DELETE(request: Request) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  await auth.writeDb.from("audit_logs").insert({ actor_id: auth.user.id, action: "archive", entity_type: table, entity_id: body.id, old_values: old, new_values: { [statusField]: archived } });
